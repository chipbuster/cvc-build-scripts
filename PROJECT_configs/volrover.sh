...
function build_project()
{
  ...
  cmake $SRC_DIR -DBUILD_QSLIM=ON -DBUILD_SURFACEMESHDECOMPOSITION=ON \
        -DBUILD_SWEETMESH_LIB=ON -DBUILD_TILING_LIB=ON -DBUILD_VOLUMEROVER_2=ON\
        -DBUILD_VOLUTILS=ON -DBUILD_MMHLS_LIB=ON -DBUILD_HLEVELSET_LIB=ON\
        -DBUILD_SUPERSECONDARYSTRUCTURES_LIB=ON
